...
class ConsoleWindow extends react.Component {
    ...
    toConsoleText(textObjs) {
        let ret = Array();
        const animationNum = Math.max(...textObjs.map((singleObj)=>singleObj.animationNum ? singleObj.animationNum : 1));
        textObjs.forEach((singleObj, index)=>{
            const animationSteps = animationNum == 1 ? 'always' : `${singleObj.animationNum ? singleObj.animationNum : 1}of${animationNum}`;
            switch(singleObj.style){
                case 'blink':
                    ret.push(react.createElement("span", {
                        key: index,
                        className: `char-pointer-${animationSteps}`
                    }, singleObj.text));
                    break;
                case 'none':
                default:
